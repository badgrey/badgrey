  ...
  render() {
    return (
      <div style={{display: 'flex', justifyContent: 'center', marginTop: '6em'}}>
         {
          this.props.volume.BricksChapters && this.props.volume.BricksChapters.map(chap => (
            <div key={chap.id} onClick={() => this.handleChapterClick(chap.title)} style={{display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              <h2>{chap.title}</h2>
